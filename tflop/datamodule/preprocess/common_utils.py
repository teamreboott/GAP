...
def generate_filled_html(gold_text_list, is_cell_filled, org_html_list):
    ...
    # Count expected </td> tags
    td_count = org_html_list.count("</td>")
    
    # Validate inputs
    if len(gold_text_list) != td_count:
        print(f"WARNING: gold_text_list length ({len(gold_text_list)}) != </td> count ({td_count})")
        # Pad gold_text_list if needed
        while len(gold_text_list) < td_count:
            gold_text_list.append("")
    
    if len(is_cell_filled) != td_count:
        print(f"WARNING: is_cell_filled length ({len(is_cell_filled)}) != </td> count ({td_count})")
        # Pad is_cell_filled if needed
        while len(is_cell_filled) < td_count:
            is_cell_filled.append(False)

    for org_html_tag in org_html_list:
        if org_html_tag != "</td>":
            filled_html_list.append(org_html_tag)
        else:
            if data_index < len(gold_text_list):
...
def get_cell_pointer_label(
    ...
    # Check if there are any coordinates to serialize
    if filtered_gold_cell_coords:
        ...
        # If no filtered coordinates, use empty lists
...
def serialize_bbox_top_left_bottom_right(coord_list, cell_text_list):
    ...
    # Handle empty coord_list
    if not coord_list:
        return [], [], []
    
    heights = [abs(x[1] - x[3]) for x in coord_list]
    if not heights or all(h == 0 for h in heights):
        # If no valid heights, return empty lists
        return [], [], []
    
    min_height = min(h for h in heights if h > 0)  # Use only positive heights
