...
def prepare_image_tensor(
    ...
    try:
        # First try to load the image to check if it's corrupted
        input_image.load()
        resized_img = resize(
            input_image.convert("RGB"), min(target_img_size)
        )  # Resized with smaller edge = min(target_img_size)
    except (OSError, IOError) as e:
        filename = getattr(input_image, 'filename', 'unknown')
        print(f"Error: Corrupted or truncated image file: {filename}")
        print(f"Error details: {str(e)}")
        raise ValueError(f"Cannot process corrupted/truncated image: {filename}. Error: {str(e)}")
    except Exception as e:
        filename = getattr(input_image, 'filename', 'unknown')
        print(f"Error resizing image: {filename}")
        print(f"Error details: {str(e)}")
        raise ValueError(f"Error in resizing image: {filename}. Error: {str(e)}")
