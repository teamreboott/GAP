...
class TFLOPDataset(Dataset):
    ...
    def _validate_sample_integrity(self, sample):
        """Validate sample data integrity to prevent index errors."""
        try:
            # Check if required fields exist
            required_fields = ['gold_coord', 'otsl_seq', 'org_html']
            for field in required_fields:
                if field not in sample:
                    raise ValueError(f"Missing required field: {field}")
            
            # Parse gold coordinates to check data consistency
            gold_coords = sample['gold_coord']
            if not isinstance(gold_coords, list):
                raise ValueError("gold_coord must be a list")
            
            # Check OTSL sequence integrity
            otsl_seq = sample.get('otsl_seq', '')
            if not isinstance(otsl_seq, str):
                raise ValueError("otsl_seq must be a string")
            
            # Count expected data cells from OTSL sequence and verify with non-empty coordinates
            if self.use_OTSL:
                c_tag_count = otsl_seq.count('C-tag')
                
                # Count total coordinates (all cells including empty ones should match C-tag count)
                total_coords = len(gold_coords)
                
                if c_tag_count != total_coords:
                    raise ValueError(f"C-tag count ({c_tag_count}) != total coordinates ({total_coords}). Expected them to match.")
                    
        except Exception as e:
            raise ValueError(f"Sample integrity validation failed: {str(e)}")
    ...
    def __getitem__(self: "TFLOPDataset", idx: int):
        ...
        try:
            ...
            # Validate sample integrity before processing
            self._validate_sample_integrity(sample)
        ...
        except (ValueError, OSError, IndexError, Exception) as e:
            print(f"\n{'='*60}")
            print(f"ERROR: Failed to load sample at index {idx}")
            ...
            try:
                data_selected = self.load_sampled_metadata(idx)
                sample = self.build_sample_dictionary(data_selected)
                print(f"Problematic file: {sample.get('file_name', 'Unknown')}")
                print(f"Dataset: {sample.get('dataset', 'Unknown')}")
                print(f"Split: {sample.get('split', 'Unknown')}")
                
                # Additional debug info for IndexError
                if isinstance(e, IndexError):
                    gold_coords = sample.get('gold_coord', [])
                    otsl_seq = sample.get('otsl_seq', '')
                    c_tag_count = otsl_seq.count('C-tag') if otsl_seq else 0
                    print(f"Gold coords length: {len(gold_coords)}")
                    print(f"C-tag count in OTSL: {c_tag_count}")
                    print(f"OTSL sequence preview: {otsl_seq[:100]}...")
            ...
            # Try next index and recursively call
            next_idx = (idx + 1) % len(self.metadata_loaded)
    ...
    def build_sample_dictionary(self, data_selected):
        ...
        # Check if this is a SynthTabNet dataset (file_name contains dataset/images/split structure)
        file_name = data_selected["file_name"]

        # Skip macOS metadata files
        if '__MACOSX' in file_name or os.path.basename(file_name).startswith('._'):
            raise ValueError(f"Skipping macOS metadata file: {file_name}")

        # Check if this is SynthTabNet data (has subdirectory structure)
        # SynthTabNet: marketing/images/val/image_xxx.png
        # PubTabNet: just image_xxx.png
        if "synthtabnet" in self.image_path:
            # SynthTabNet format: dataset/images/split/filename.png
            # Don't add self.split since it's already in the file_name
            image_path = os.path.join(
                self.image_path,
                file_name
            )
        else:
            # Original PubTabNet format
            image_path = os.path.join(
                self.image_path,
                self.split,
                file_name
        ...
        if file_size == 0:
            ...
            raise ValueError(f"Image file is suspiciously small ({file_size} bytes): {image_path}")
