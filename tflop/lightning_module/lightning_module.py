...
class TFLOPModelPLModule(pl.LightningModule):
    ...
    def load_pretrained_weights(self):
        ...
        if self.config.get("pretrained_model_name_or_path", False):
            ...
            # Resize token embeddings after loading pretrained weights
            # This ensures the embedding layer matches the tokenizer vocabulary size
            if hasattr(self.model.decoder, 'model') and hasattr(self.model.decoder.model, 'resize_token_embeddings'):
                self.model.decoder.model.resize_token_embeddings(len(self.tokenizer))
                actual_embed_size = self.model.decoder.model.model.decoder.embed_tokens.num_embeddings
                actual_lm_head_size = self.model.decoder.model.lm_head.out_features
                print(f"Resized token embeddings to {len(self.tokenizer)} after loading pretrained weights")
                print(f"Actual embedding size after resize: {actual_embed_size}")
                print(f"Actual lm_head size after resize: {actual_lm_head_size}")
                assert actual_embed_size == len(self.tokenizer), f"Embedding size mismatch: {actual_embed_size} != {len(self.tokenizer)}"
                assert actual_lm_head_size == len(self.tokenizer), f"LM head size mismatch: {actual_lm_head_size} != {len(self.tokenizer)}"
